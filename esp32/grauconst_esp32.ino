...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        if (advertisedDevice.haveManufacturerData()) {
            ...
            // O Inkbird IBS-TH2 costuma ter o nome "tps" ou "sps"
...
void setup() {
  Serial.begin(115200);
  delay(500); // Delay maior para estabilizar serial
  bootCount++;
  Serial.printf("\n[GRAUCONST] Boot #%d | FW %s\n", bootCount, FIRMWARE_VERSION);

  // Importante: Inicializar WiFi antes de pegar o MAC
  WiFi.mode(WIFI_STA);
  delay(100);
...
String montarPayload(int rssi, InkbirdData ink) {
  ...
  // Como o usuário não quer DHT22, enviamos null para temperatura/umidade locais
